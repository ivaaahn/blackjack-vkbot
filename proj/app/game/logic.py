...
async def personal_player_statistic(
    ctx: GameCtxProxy, access: "GAccessors", p: PlayerModel
) -> str:
...
async def handle_pers_statistic(ctx: GameCtxProxy, access: "GAccessors") -> None:
    ...
    await send(
        ctx, access, await personal_player_statistic(ctx, access, player), Kbds.START
    )
    ctx.state = States.WAITING_FOR_START_CHOICE
...
async def handle_bonus(ctx: GameCtxProxy, access: "GAccessors") -> None:
    ...
    if created:
        ...
        Следующий бонус будет доступен через {pretty_time_delta(td(minutes=sets.bonus_period))}"""
        await access.players.give_bonus(
...
async def fetch_user_info(
    ctx: GameCtxProxy, access: "GAccessors", start_cash: float
    ...
    flag = False
    db_user_data = await access.players.get_player_by_vk_id(
    ...
    return flag, await access.players.get_player_by_vk_id(
        chat_id=ctx.chat_id, vk_id=ctx.msg.from_id
    )
...
async def hand_out_cards(ctx: GameCtxProxy, access: "GAccessors"):
    ...
    try:
        ...
        txt = f"%0A%0A".join(
            [f"◾ {p}, вот твои карты:%0A{p.cards_info}" for p in g.players_and_dealer]
        )
...
async def handle_bj_pick_up11_action(
    ctx: GameCtxProxy, access: "GAccessors", player: Player
) -> bool:
...
async def handle_bj_pick_up32_action(
    ctx: GameCtxProxy, access: "GAccessors", player: Player
) -> bool:
    return True


async def handle_bj_wait_action(
    ctx: GameCtxProxy, access: "GAccessors", player: Player
) -> bool:
...
async def init_game(
    ctx: GameCtxProxy, access: "GAccessors", num_of_players: int
) -> None:
    sets = await access.settings.get(_id=0)
...
async def place_bet(ctx: GameCtxProxy, access: "GAccessors", player: Player) -> None:
    if (bet := parse_bet_expr(ctx.msg.text)) is None or bet < 0:
        ...
        answer = (
            f"{player}, на счете недостаточно денег, дружище. На счете: {player.cash}"
        )
...
async def calculate_stats(
    ...
    p = await access.players.get_player_by_vk_id(
...
async def update_players_data(ctx: GameCtxProxy, access: "GAccessors") -> None:
    for player in ctx.game.players:
        ...
        await access.players.update_after_game(
