...
def group_by_chat_pipeline(max_players: int = 5) -> list[dict]:
    return [
        {"$group": {"_id": "$chat_id", "players": {"$push": "$$ROOT"}}},
        {"$addFields": {"number_of_players": {"$size": "$players"}}},
        {
            "$project": {
                "_id": 0,
                "chat_id": "$_id",
...
def chat_pagination_pipeline(
    offset: int, limit: int, order_by: Optional[str], order_type: int
) -> list[dict]:
    pipeline = [
